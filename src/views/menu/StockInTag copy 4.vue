                  ...
                  <v-card-text>
                    <v-container>
                      ...
                      <v-row>
                        ...
                        <v-col cols="12" md="4">
                          <v-autocomplete
                            @focus="listGridForCheckByReagentName"
...
export default {
  ...
  computed: {
    ...
    fromReagNameDisp() {
      if (this.editedItem.stockInTag_reagName != '') {
        ...
        console.log("fromReagNameDisp, result 1...", result);
  ...
  methods: {
    ...
    /*
    filterResults() {
      let  _items = this.autocomplete_items.map(function (item, index, array) {
        return item;
      });

      console.log("_items", _items);
      let kk=this.editedItem.stockInTag_reagName
      this.autocomplete_items = _items.filter(function (item, index, array) {
        return item.indexOf(kk) > -1;
      });
    },

    setResult(result) {
      var vm = this;
      if (this.autocomplete_items.length !== 0) {
        this.editedItem.stockInTag_reagName = result;
        this.arrowCounter = -1;
        this.isOpen = false;
      }
    },
