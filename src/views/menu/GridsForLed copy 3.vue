...
export default {
  ...
  methods: {
    getComp() {
      ...
      //let el_v1 = document.getElementsByClassName('v1').length;
      //console.log("getcomP(): ", el, el_v1)
      for (let i=0; i< el; i++) {
        ...
        //.getElementsByClassName('v1')
        //console.log("getcomP(): ", v22)

        //.style['left'] = 25 * this.segments[i].size + 'px';
      }
    },
    ...
    toggleLed() {
      ...
      for (let i=0; i<temp_seg_size; i++) {
        if (this.range[0] <= this.segments[i].range0 || this.range0 <= this.segments[i].range1 ||
            this.range[1] <= this.segments[i].range0 || this.range1 <= this.segments[i].range1) {
          this.rangErrMsg='格位資料設定錯誤!'
          return;
        }
      }
      ...
      //console.log("b temp_size", temp_size, this.range[0], this.range[1])
      let obj= {
        id: this.init_id,
      ...
      //console.log("a temp_size", obj.range)
